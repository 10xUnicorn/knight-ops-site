...
- **Still required by Daniel:** Supabase secrets **`STRIPE_SECRET_KEY`** (restricted key: Connect accounts/account links/transfers write, balance read) and **`STRIPE_REVENUE_WEBHOOK_SECRET`**, plus a Stripe webhook endpoint on `charge.succeeded` → `/functions/v1/stripe-revenue-webhook`. Until then Stripe onboarding/payouts return a clear error; everything else works.
