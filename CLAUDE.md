...
- **Polish wave (same day):** admin project-detail "Add/Edit Team Member" modal restyled — permissions now a clean 2-col grid of clickable label+checkbox rows in a bordered group box (`pmPermGrid()`); same fields/classes, no functional change.
- **Admin Resources view:** every resource with a url/file_url gets 👁 View + ⬇ Download buttons (cross-origin storage URLs fetch→blob→download via `dlResource()`); global library rows (is_global=true) show a gold "Library" badge + category chip — the seeded continuity/security PDFs are all visible and viewable here.
- **Auth/activation:** auth.html supports `?redirect=/path` (same-origin only, wins over role redirect, carried through magic-link emailRedirectTo) + "First time here?" magic-link activation hint under the login form; client-hub Ownership sign-in button now links to `/auth?redirect=/portal`.
- **portal.html:** codebase-download button is UI-gated to the client OWNER (projects.client_id match or project_clients link for the clients row where profile_id = session user); other viewers see "Codebase export is available to the project owner." (hub-data v4 already enforces server-side).
- **Env export:** `project_credentials.env_key` (admin-only — never selected/rendered in client-hub) — Add Credential modal gains an Env Key input auto-filled from the label via `ENV_KEY_DEFAULTS`/`envKeyFromLabel()` (e.g. Anthropic API→ANTHROPIC_API_KEY, Supabase URL→NEXT_PUBLIC_SUPABASE_URL; fallback: uppercase/underscore + _KEY if label ends in API); Project Credentials card gains "⬇ Export .env" → 100% client-side supabase select → Blob download as `.env.<project-slug>` (values quoted if they contain spaces/#; never sent to any service or AI call) + Vercel import caption.
- **Resource library content:** 3 new PDFs (Master Guide, Bug/Feature request process, Credentials policy) + compliance PDFs v2 with when-do-you-need-this sections + continuity plan v3 naming Tim Wolf as continuity partner.
