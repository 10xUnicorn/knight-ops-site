...
## Changelog — 2026-09-03 (GSC audit: the blog had no crawl path and no CTA)

- **GSC (Jun 1 to Aug 31): 26 clicks / 4,040 impressions / 0.6% CTR / avg pos 16.2. 182 indexed vs 470 not indexed (182 "Discovered, not indexed", 124 "page with redirect").** Root causes were structural, not content volume: `/blog` rendered every card as `<div onclick>` (zero `<a href>`, so Googlebot only found posts via sitemap), rendered posts had **no CTA at all**, 281 posts linked to bare `knightops.biz` (every link a 308), and static sitemap `lastmod` was stamped "today" on every request (Google ignores it).
- **Fixed (commit `aaa592b`):** new `api/blog-index.js` server-renders `/blog` (+ `?page=N`, rel prev/next, CollectionPage schema) with real links; `vercel.json` gained ONE rewrite `/blog → /api/blog-index` (surgical, Rule 1 honoured); `api/blog.js` injects the **complimentary Tech Discovery Call CTA box** (`/book?src=blog&post=<slug>`) plus a mid-post callout before the 3rd H2, and 3 same-cluster related posts as real `<a>` links; `blog.html` keeps SSR content (`data-ssr`) instead of re-rendering it, `koWithCta()` adds the CTA on client-side navigations, and all cards now contain an `<a href>`; `api/sitemap.js` uses a fixed `STATIC_LASTMOD` (bump it when static pages change); new `api/indexnow.js` (+ key file `a7c3e9f1b2d4485f9e6c0b1d2f3a4c5e.txt`) submits recent posts to IndexNow (Bing/ChatGPT search); first call returned 403 SiteVerificationNotCompleted, expected until IndexNow fetches the key.
- **Data fixed in Supabase:** 301 posts updated: bare-host links → `https://www.knightops.biz/...`, Roundtable time → Thursdays 12pm PT / 3pm ET (0 left of either).
- **Same day, commit `5f7bb75`:** three commercial pages built from the `for-agencies.html` template: `/for-financial-advisors` (GSC already shows advisor-workflow impressions), `/for-eos-companies` (EOS/Traction adjacency: "eos implementer" 2,400/mo KD 15; EOS Implementer partner angle), and `/founder-bottleneck-score` (the **Founder Bottleneck Index**: 12 questions, 6 dimensions, 0–100 score, tier, first 90-day move; optional email capture via the existing `gravity-capture` edge fn with `capture_tool='founder_bottleneck_index'`, CTA to `/book?src=fbi&score=&tier=&dim=`). All three in `STATIC_PAGES`. **Bing Webmaster Tools verified** via `msvalidate.01` meta on `index.html` (account dknightunicorn@gmail.com; GSC lives under daniel@unicornuniverse.io, so the GSC import found nothing). IndexNow key verified: `/api/indexnow?all=1` returned **200 for 418 URLs**. GSC: indexing requested on 8 URLs, "Validate fix" started on all four not-indexed buckets (451 URLs).
- **Scheduled tasks re-aimed** at the $5M–$50M organization ICP and the keyword tiers (fractional chief AI officer, business operating system, operations dashboard): aeo-writer now 1x/day, cross-site Mon/Wed/Fri, aeo-meta-optimizer re-enabled and reads GSC via Chrome weekly, new `gsc-index-request-daily`, `blog-refresh-optimizer-daily`, `network-link-integrity-weekly`. Governing doc: `Obsidian Data/Knight Ops/AEO-GSC-Override-2026-09-03.md`.
...
- **`10xUnicorn` IS A GITHUB *USER* ACCOUNT, NOT AN ORGANIZATION.** `GET /orgs/10xUnicorn/repos` returns 404 — that was the `github_404` the Sync GitHub button first showed. `automations-sync` v2+ asks `/users/<name>` for the type and uses `/user/repos?affiliation=owner` (private repos included) for a user. v3 also deletes rows for repos that no longer exist on GitHub (upsert alone kept `planet-calm-dashboard` alive after Daniel deleted it) and matches repos→projects in SQL (`ko_github_repos_match()`) because the JS-side `.not('status','in',…)` filter returned zero projects and the first real sync matched 0 of 24. Use two `.neq()` calls for enum columns.
- **Two Planet Calm repos were a leftover shell, not a fork:** `planet-calm-dashboard` (public, 0 KB, created 9h before the real `planet-calm` on 2026-06-25, never committed to) — deleted by Daniel 2026-09-05. Same class, reversed: `nightvibe-workshop` (private) is the empty one and `night-vibe-workshop` (public) is real.
- **`knight-ops-linkedin-promo` was still promoting the Blueprint Session at $750** with `$80M+` and "24hr fastest deploy" — the Cowork audit prompt missed it because it never emits the `/book-tech-call` URL, it names the offer in prose. Repointed to the free Tech Discovery Call at `/book` with the canonical four stats; Friday "slots filling up" removed. Backup `SKILL.md.bak-20260904`.
