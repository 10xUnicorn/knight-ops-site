...
- **BLOCKED until Daniel sets Connect site links:** live account links (onboarding URLs) cannot be created until URLs are supplied at dashboard.stripe.com/settings/connect/site-links for notification banner, account management, payouts, payments, balance, documents. Account *creation* works; only the hosted onboarding link is blocked. This is a platform-wide Connect setting shared with Headliner (additive — it fills empty fields; Headliner's Standard accounts connect via OAuth and don't use these links).
