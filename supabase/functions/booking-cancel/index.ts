// booking-cancel v13 — token-gated cancel; deletes the Google event and sends a METHOD:CANCEL invite from daniel@knightops.biz.
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.39.3";
import { deleteBookingEvent } from "../_shared/gcal.ts";
import { hostNotify, send, fmtDate, fmtTime, esc, HOST_NOTIFY, buildIcs, icsAttachment } from "../_shared/booking-mail.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await send([bk.booker_email], `Cancelled: ${t.name} on ${fmtDate(bk.start_time, tz)}`, guestHtml, "Daniel Knight", [icsAttachment(buildIcs(bk, t, "CANCEL", (bk.reschedule_count || 0) + 1), "cancel.ics")]).catch(() => null);
